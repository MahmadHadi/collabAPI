...
const showMenu = () => {
  ...
  console.log("4.  Update Request");
  console.log("5.  Delete Request");
  console.log("6.  Seed Dummy Data");
  console.log("7.  Exit");
  console.log("=".repeat(50));
};
...
const createRequest = async () => {
  ...
  const title = await askQuestion("Title (default: new request): ") || "new request";
  ...
  const headersInput = await askQuestion("Headers (key:value, comma separated, e.g., Auth:token,Content-Type:json): ");
  const headers = headersInput ? headersInput.split(",").map(h => {
    const [key, value] = h.split(":");
    return { key: key.trim(), value: value.trim() };
  }) : [];
  
  const queryInput = await askQuestion("Query params (key:value, comma separated): ");
  const query = queryInput ? queryInput.split(",").map(q => {
...
const viewAllRequests = async () => {
  ...
  try {
    ...
    requests.forEach((req, index) => {
      ...
      console.log(`   Headers: ${req.req_headers.length} | Query: ${req.req_query.length}`);
      console.log("-".repeat(30));
    });
  } catch (error) {
...
const updateRequest = async () => {
  ...
  try {
    ...
    const method = await askQuestion("New method (GET/POST/PUT/DELETE/PATCH): ") || existing.req_method;
    
    const headersInput = await askQuestion(`Current headers: ${JSON.stringify(existing.req_headers)}\nNew headers (key:value, comma separated): `);
    const headers = headersInput ? headersInput.split(",").map(h => {
      const [key, value] = h.split(":");
      return { key: key.trim(), value: value.trim() };
    }) : existing.req_headers;
    
    const queryInput = await askQuestion(`Current query: ${JSON.stringify(existing.req_query)}\nNew query (key:value, comma separated): `);
    const query = queryInput ? queryInput.split(",").map(q => {
      const [key, value] = q.split(":");
      return { key: key.trim(), value: value.trim() };
    }) : existing.req_query;
    ...
    const body = await askQuestion("New body (JSON string): ") || existing.req_body;
